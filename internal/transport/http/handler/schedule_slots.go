...
// 统一成结构体是为了让幂等协调可以把第一次结果原样保存并重放（含 Location）。
...
func (h *ScheduleSlotHandler) CreateSlot(c *gin.Context) {
	...
	idemKey := idempotencyKey(userID, planID, c.FullPath(), key)
...
func idempotencyKey(userID, planID int64, fullPath, key string) string {
