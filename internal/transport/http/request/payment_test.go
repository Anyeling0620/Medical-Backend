// 支付域请求绑定单测：POST /api/v1/payments 与 POST /api/v1/payments/orders
// 的请求体严格解析（spec/04-api-contract.md §6.5、§6.9、§12.4）。
//
// 覆盖空体、未知字段、registrationId 缺失/非正/类型错误、多余内容与合法值；
// method 的取值校验属于业务语义（用例层判定），本层只做必传与类型校验；
// 创建接口只接收 registrationId，同样在绑定层收敛为面向用户的中文文案。
package request
...
// bindPaymentCreateInTest 在最小 gin 路由上执行 BindPaymentCreate 并返回绑定结果。
// 辅助端点不写响应体，用例只关心绑定结果本身。
func bindPaymentCreateInTest(t *testing.T, body string) (PaymentCreateRequest, error) {
	t.Helper()
	gin.SetMode(gin.TestMode)

	var got PaymentCreateRequest
	var bindErr error
	engine := gin.New()
	engine.POST("/api/v1/payments/orders", func(c *gin.Context) {
		got, bindErr = BindPaymentCreate(c)
	})
...
// TestBindPaymentCreateRejectsMissingOrInvalidFields 覆盖创建支付订单请求体的错误分支：
// 空体、未知字段、registrationId 缺失/为 0/为负与多余内容（契约 §6.9、§12.4）。
func TestBindPaymentCreateRejectsMissingOrInvalidFields(t *testing.T) {
	cases := []struct {
		name    string
		body    string
		wantErr string
		...
		{
			name:    "未知字段",
			body:    `{"registrationId":1001,"extra":true}`,
			wantErr: "请求体格式不正确（包含未知字段）",
		},
		{name: "registrationId 缺失", body: `{}`, wantErr: "registrationId 为必传字段"},
		{name: "registrationId 为 0", body: `{"registrationId":0}`, wantErr: "registrationId 必须为正整数"},
		{name: "registrationId 为负数", body: `{"registrationId":-1}`, wantErr: "registrationId 必须为正整数"},
		{name: "合法 JSON 后跟多余内容", body: `{"registrationId":1001}{"registrationId":1002}`, wantErr: "请求体包含多余内容"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := bindPaymentCreateInTest(t, tc.body)
			if err == nil {
				t.Fatalf("期望绑定失败（%s），实际 err = nil", tc.wantErr)
			}
			if err.Error() != tc.wantErr {
...
// TestBindPaymentCreateAcceptsValidBody 合法请求体必须绑定成功并原样保留字段。
func TestBindPaymentCreateAcceptsValidBody(t *testing.T) {
	body, err := bindPaymentCreateInTest(t, `{"registrationId":1001}`)
	if err != nil {
		t.Fatalf("合法请求体不应报错，实际 %v", err)
	}
	if body.RegistrationID == nil {
...
// TestBindPaymentCreateKeepsJSONSyntaxAndTypeErrors 覆盖 JSON 语法/类型错误：
// 这两类错误保留原始类型，由 handler 映射为 400 REQUEST_INVALID_JSON，
// 不得被归一为「未知字段」这类 422 文案。
func TestBindPaymentCreateKeepsJSONSyntaxAndTypeErrors(t *testing.T) {
	cases := []struct {
		name       string
		body       string
		wantSyntax bool
...
// TestBindPaymentCreateKeepsTruncatedJSONError 请求体被截断时返回 io.ErrUnexpectedEOF，
// 由 handler 映射为 400 REQUEST_INVALID_JSON。
func TestBindPaymentCreateKeepsTruncatedJSONError(t *testing.T) {
	_, err := bindPaymentCreateInTest(t, `{"registrationId":`)
	if err == nil {
		t.Fatal("截断的 JSON 期望绑定失败，实际 err = nil")
	}
	if !errors.Is(err, io.ErrUnexpectedEOF) {
